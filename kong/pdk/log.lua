...
local phases_with_ctx =
    phase_checker.new(PHASES.certificate,
                      PHASES.rewrite,
                      PHASES.access,
...
local function get_default_serialize_values()
  if ngx.config.subsystem == "http" then
    return {
      ["request.headers.authorization"] = { value = "REDACTED", mode = "replace" },
      ["request.headers.proxy-authorization"] = { value = "REDACTED", mode = "replace" },
    }
  end
...
--
local set_serialize_value

do

  local function is_valid_value(v)
    local t = type(v)
  ...
  set_serialize_value = function(key, value, options)
    ...
    if not is_valid_value(value) then
      error("value must be a number, string, boolean or a table made with only numbers, string and booleans", 2)
    end
...
do
  ...
  local function edit_result(ctx, root)
    ...
    local mode, new_value, subkeys, node, subkey, last_subkey, existing_value
    for key, value_mode in pairs(serialize_values) do
      mode = value_mode.mode
      new_value = value_mode.value
      ...
      for i = 1, #subkeys - 1 do -- note that last subkey is treated differently, below
        ...
        if mode == "set" or mode == "add" then
          node[subkeys[i]] = node[subkeys[i]] or {} -- set and add modes create subtables
        end
        if type(node[subkeys[i]]) ~= "table" then
          error("The key " .. key .. "could not be used as a serialize value. " ..
                "Subkey '" .. subkey .. "' is not a table")
        end
      end
      last_subkey = subkeys[#subkeys]
      existing_value = node[last_subkey]
      if (mode == "set")
      or (mode == "add" and not existing_value)
      or (mode == "replace" and existing_value)
      then
